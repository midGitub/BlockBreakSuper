module framework.session {
	...
	import type = framework.util.type;
	...
	export class dataInfo {
		...
		public string(type: framework.util.formatType = framework.util.formatType.Normal, format: string = null, maxLength: number = 0) {
			var value = this.value();
			if (value == null) {
				return "";
			}
			if (util.type.isNumber(value)) {
				return util.format.number(value, type, format);
			}
			return value;
		}
